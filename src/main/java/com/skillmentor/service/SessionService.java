...
public class SessionService {
        ...
        public SessionResponse enrollSession(Long studentId, EnrollSessionRequest request) {
                ...
                // Fetch all sessions for this mentor on this date
                Long mid = mentor.getId();
                List<Session> mentorSessionsOnDate = sessionRepository.findByMentorIdAndSessionDate(
                                mid != null ? mid : 0L,
                                request.getSessionDate());

                // Check for overlaps in Java (DB agnostic)
                boolean hasOverlap = mentorSessionsOnDate.stream()
                                .filter(s -> !s.getStatus().equals(SessionStatus.CANCELLED))
                                .anyMatch(s -> {
                                        LocalTime existingStart = s.getSessionTime();
                                        LocalTime existingEnd = existingStart.plusMinutes(s.getDurationMinutes());

                                        // Overlap condition: (StartA < EndB) AND (EndA > StartB)
                                        return existingStart.isBefore(endTime)
                                                        && existingEnd.isAfter(request.getSessionTime());
                                });
        ...
        public SessionResponse confirmPayment(Long sessionId) {
                ...
                // Validate: Can only confirm PENDING or UNDER_REVIEW payments
                if (session.getPaymentStatus() != PaymentStatus.PENDING &&
                                session.getPaymentStatus() != PaymentStatus.UNDER_REVIEW) {
                        throw new IllegalArgumentException(
                                        "Can only confirm pending or under review payments");
                }
         ...
         * UPLOAD PAYMENT SLIP
         *
         * Student uploads an image of their payment receipt.
         * The image is stored in Supabase under "payment-slips/" folder.
         */
        @Transactional
        public SessionResponse uploadPaymentSlip(Long sessionId, org.springframework.web.multipart.MultipartFile file) {
                Session session = sessionRepository.findById(sessionId)
                                .orElseThrow(() -> new ResourceNotFoundException("Session not found"));

                // 1. Upload file to Supabase
                String url = storageService.uploadImage(file, "payment-slips");

                if (url == null) {
                        throw new IllegalStateException("Failed to upload payment slip");
                }

                // 2. Update session with URL and change status
                session.setPaymentSlipUrl(url);
                session.setPaymentStatus(PaymentStatus.UNDER_REVIEW);

                // Optional: You might want to update paymentStatus to a
                // "WAITING_FOR_CONFIRMATION" style status
        ...
        private SessionResponse convertToResponse(Session session) {
                return SessionResponse.builder()
                                ...
                                .studentName(formatName(session.getStudent().getFirstName(),
                                ...
                                .paymentSlipUrl(session.getPaymentSlipUrl())
